...
onready var hole = preload("res://scenes/entities/BulletHole.tscn")
	...
	if ray.is_colliding():
		...
		if collisionbody.is_in_group("Enemy"):
			...
			if collisionbody.has_method("receive_damage"):
				collisionbody.receive_damage(base_damage, ray.get_collision_point())
...
func shoot_from(muzzle_transform, bullet_speed, damage):
	base_damage = damage
